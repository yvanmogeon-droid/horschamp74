...
RÈGLE DE SORTIE ABSOLUE (la plus importante de toutes) : ta réponse contient UNIQUEMENT la brève finale. La PREMIÈRE LIGNE est le TITRE en MAJUSCULES, sans rien avant. Tu ne décris JAMAIS ton raisonnement, ton tri ni ton choix d'article. Sont STRICTEMENT INTERDITS, surtout en tête de réponse : « En analysant le flux », « En croisant le flux », « voici mon analyse », « mon analyse : », toute énumération du type « Art. 1 … Art. 2 … », les mentions « → hors sujet », « non publié », « pertinent ». Tout ce qui ressemble à une justification de ton choix doit rester dans ta tête. Si aucun article ne convient, réponds EXACTEMENT et seulement : AUCUN SUJET.
...
- Écris-le en MAJUSCULES, sur la toute première ligne. Ne donne qu'UN seul titre, ta meilleure version, jamais de variantes, et RIEN avant lui.
...
// ─── Détection du raisonnement de tri qui a fui dans la sortie ─────────────────
// Le bug historique : certains jours le modèle commence par « En croisant le flux…
// voici mon analyse : - Art. 1 … » et cette phrase devient le titre de la brève.
// Ces garde-fous repèrent ce parasite et permettent de le retirer ou de rejeter.
const REGEX_PARASITE = /(en analysant|en croisant|voici mon analyse|mon analyse\s*:|analyse du flux|→\s*hors sujet|\bhors sujet\b|non publié|^\s*-?\s*art\.?\s*\d+\s*[:(]|^\s*article\s+\d+\s*:)/i;

// Un vrai titre Hors Champ est court et écrit en MAJUSCULES.
function ligneEstTitrePlausible(l) {
const s = l.trim();
if (s.length < 12 || s.length > 140) return false;
if (REGEX_PARASITE.test(s)) return false;
const lettres = s.replace(/[^A-Za-zÀ-ÿ]/g, '');
if (lettres.length === 0) return false;
const majuscules = s.replace(/[^A-ZÀ-Þ]/g, '').length;
return majuscules / lettres.length >= 0.6; // ≥60 % de majuscules
}

// Retire tout préambule/raisonnement avant le vrai titre.
// Renvoie null si aucune ligne ne ressemble à un titre (= sortie à rejeter).
function nettoyerPreambule(texte) {
const lignes = texte.split('\n');
const idx = lignes.findIndex(l => l.trim() && ligneEstTitrePlausible(l));
if (idx === -1) return null;
return lignes.slice(idx).join('\n').trim();
}
// ─── parseBreve ───────────────────────────────────────────────────────────────
...
// ─── Génération + garde-fou anti-préambule ───────────────────────────────
// On tente jusqu'à 2 fois. Si la sortie ressemble à du raisonnement de tri
// (« En croisant le flux… », « Art. 1… ») et qu'aucun vrai titre n'est
// récupérable, on NE PUBLIE RIEN : la brève de la veille reste en ligne,
// plutôt que de publier la « statistique de daube ».
let parsed = null;
let breve = null;
for (let tentative = 1; tentative <= 2; tentative++) {
const rappel = tentative === 1
? ''
: '\n\nRAPPEL STRICT : ta réponse précédente contenait du raisonnement interdit. Réponds UNIQUEMENT par la brève. PREMIÈRE LIGNE = le TITRE en MAJUSCULES. Aucun préambule, aucune analyse, aucune énumération d\'articles. Ou bien « AUCUN SUJET ».';
const texteRaw = await callClaude(userMessage + rappel);
if (texteRaw.toUpperCase().includes('AUCUN SUJET')) {
console.log('ℹ️ Claude : AUCUN SUJET — arrêt.');
process.exit(0);
}
const p = parseBreve(texteRaw);
const nettoye = nettoyerPreambule(p.breve);
const premiereLigne = nettoye ? (nettoye.split('\n')[0] || '') : '';
if (nettoye && !REGEX_PARASITE.test(premiereLigne)) {
parsed = p;
parsed.breve = nettoye; // brève débarrassée de tout préambule
breve = nettoye;
break;
}
console.error(`🛑 Sortie ${tentative} rejetée : raisonnement de tri détecté, pas de titre exploitable.`);
}
if (!breve) {
console.error('🛑 Deux sorties invalides d\'affilée — rien n\'est publié ni envoyé. La brève de la veille reste en ligne.');
process.exit(0);
}

const { category, source, articleIndex, imageQuery, lieu } = parsed;
